...
public class Canvas extends Component implements Accessible {
    ...
    public Graphics getGraphics() {
        if(checked && paintCallback == null){
            return super.getGraphics();
        }
        if(paintCallback == null){
            paintCallback = BotControl.getPaintCallback(this);
            checked = true;
        }
        return paintCallback.getClientGraphics();
    }
     ...
     * @since JDK1.0
     *
     * Hacked so world map works ;)
     */
    @Override
    public Image createImage(int width, int height) {
        return new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);    //To change body of overridden methods use File | Settings | File Templates.
    }
}